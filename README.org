...
?block_until(#{kind => foo_init_complete, pid := Pid}, Timeout), %% Timeout is optional, defaults to `infinity'
do_stuff(Pid)
#+END_SRC

This macro returns either the first matched event or atom =timeout=.
